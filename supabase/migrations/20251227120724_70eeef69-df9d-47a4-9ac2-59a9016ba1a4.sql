-- Fix the create_notification call order in process_team_tournament_join function
-- The function signature is: create_notification(p_user_id uuid, p_type text, p_title text, p_message text, p_related_id uuid)
...
BEGIN
  ...
  FOREACH v_member_id IN ARRAY p_team_member_ids LOOP
    ...
    -- Send notification to each member with CORRECT parameter order
    -- Signature: create_notification(p_user_id uuid, p_type text, p_title text, p_message text, p_related_id uuid)
