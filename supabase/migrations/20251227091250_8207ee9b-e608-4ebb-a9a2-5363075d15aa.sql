-- Fix ambiguous RPC due to duplicated overloaded function signatures
-- Keep the (p_leader_id, p_tournament_id, p_team_member_ids, p_team_name) version and drop the conflicting overload.

DROP FUNCTION IF EXISTS public.process_team_tournament_join(
...
-- Ensure the intended signature exists (no-op if already present)
-- (This statement will error if it already exists with identical signature, so we guard via CREATE OR REPLACE)
...
BEGIN
  ...
  SET current_prize_pool = COALESCE(current_prize_pool, 0) + v_prize_pool_share,
      ...
      joined_users = COALESCE(joined_users, ARRAY[]::text[]) || (SELECT array_agg(m::text) FROM unnest(p_team_member_ids) m),
...
REVOKE ALL ON FUNCTION public.process_team_tournament_join(uuid, uuid, uuid[], text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.process_team_tournament_join(uuid, uuid, uuid[], text) TO authenticated;