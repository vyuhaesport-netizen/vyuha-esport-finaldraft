-- Update the trigger function to use 'commission' type (allowed by check constraint)
...
BEGIN
  ...
  -- Check if this is a qualifying action (not commission itself to avoid recursion)
  IF NEW.type NOT IN ('deposit', 'admin_credit', 'tournament_join', 'prize', 'prize_won', 'entry_fee') THEN
    RETURN NEW;
  END IF;
  ...
  -- Create wallet transaction for commission using 'commission' type (allowed)
  ...
  VALUES (
    ...
    'commission',
    'completed',
    'Collab Referral Commission: User qualified via ' || NEW.type
  );
...
DROP TRIGGER IF EXISTS trigger_collab_on_wallet ON public.wallet_transactions;
CREATE TRIGGER trigger_collab_on_wallet
AFTER INSERT ON public.wallet_transactions
FOR EACH ROW
EXECUTE FUNCTION public.trigger_collab_qualification();