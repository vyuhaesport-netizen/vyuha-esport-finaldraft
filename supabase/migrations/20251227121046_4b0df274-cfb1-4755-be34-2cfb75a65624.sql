-- Fix the COALESCE type mismatch in process_team_tournament_join
...
BEGIN
  ...
  -- Convert uuid array to text array for joined_users
  SELECT array_agg(m::text) INTO v_member_ids_text FROM unnest(p_team_member_ids) m;
  ...
  SET current_prize_pool = COALESCE(current_prize_pool, 0) + v_prize_pool_share,
      ...
      joined_users = COALESCE(joined_users, ARRAY[]::text[]) || v_member_ids_text,
